...
class PartialFC(Module):
    ...
    def __init__(self, rank, local_rank, world_size, batch_size, resume,
                 margin_softmax, num_classes, sample_rate=1.0, embedding_size=512, prefix="./", loss_type = 'arcface'):
        """
        rank: int
        ...
        if resume:
            try:
                ...
                print('Error loadding: ', self.weight_name)
                self.weight = torch.normal(0, 0.01, (self.num_local, self.embedding_size), device=self.device)
                self.weight_mom: torch.Tensor = torch.zeros_like(self.weight)
        else:
            self.weight = torch.normal(0, 0.01, (self.num_local, self.embedding_size), device=self.device)
    ...
    def save_params(self, folder = None):
        """ Save softmax weight for each rank on prefix
        """
        if folder is not None:
            weight_name = os.path.join(folder, "rank_{}_softmax_weight.pt".format(self.rank))
            weight_mom_name = os.path.join(folder, "rank_{}_softmax_weight_mom.pt".format(self.rank))
            torch.save(self.weight.data, weight_name)
    ...
    def prepare(self, label, optimizer):
        ...
        with torch.cuda.stream(self.stream):
            ...
            # print('Local rank {}: total_label in {}'.format(self.local_rank, total_label), label.shape)
            self.sample(total_label)
            # print('Local rank {}: total_label out {}'.format(self.local_rank, total_label), label.shape)
    ...
    def forward_backward(self, label, features, optimizer, norms = None):
        """
        ...
        if self.loss_type == 'adaface':
            total_norms = torch.zeros(
                size=[self.batch_size * self.world_size, 1], device=self.device)
            dist.all_gather(list(total_norms.chunk(self.world_size, dim=0)), norms.data)
            total_norms.requires_grad = True

            # print('Local rank {}: norms {}'.format(self.local_rank, norms), norms.shape)
            # print('Local rank {}: total norms {}'.format(self.local_rank, total_norms), total_norms.shape)
        logits = self.forward(total_features, norm_weight)
        if self.loss_type == 'adaface':
            assert norms is not None, "Get no input for norms variable"
            logits = self.margin_softmax(logits, total_norms, total_label)
        else:
